...
fn test_weather_command() {
    ...
    if !output.status.success() {
        ...
        assert!(
            has_cache_error || has_network_error || has_location_error,
            "Expected cache, network, or location error, got: {}",
            combined_output
        );
    }
...
fn test_weather_command_verbose() {
    ...
    if output.status.success() {
        ...
        assert!(
            combined_output.contains("Weather Forecast")
        ...
        assert!(
            ...
            "Failed command should show verbose diagnostics, got: {}",
            combined_output
        );
    }
}
...
fn test_weather_no_api_key_success() {
    ...
    if !output.status.success() {
        ...
        assert!(
            ...
            "Should not require API key with OpenMeteo, got: {}",
            combined_output
        );
    }
