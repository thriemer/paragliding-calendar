...
use crate::{
    adapters::activities::{
        events::outdoor_active as event_parser, hiking::outdoor_active as tour_parser,
    },
    domain::ports::{EventRepository, OutdoorTourRepository},
};
...
pub async fn sync_tours(repo: &dyn OutdoorTourRepository) -> Result<usize> {
    ...
    let tours = tokio::task::spawn_blocking(move || {
        parse_zip(&parse_path, tour_parser::parse_tour)
    })
    .await??;
...
pub async fn sync_events(repo: &dyn EventRepository) -> Result<usize> {
    ...
    let events = tokio::task::spawn_blocking(move || {
        parse_zip(&parse_path, event_parser::parse_event)
    })
    .await??;
